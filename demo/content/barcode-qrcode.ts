...
const verticalBarcodeIntro = `orientation: 'vertical'/'vertical-reversed' rotates the whole rendered barcode (bars and the text line together) within its box — the same natural left-to-right drawing, just wrapped in a coordinate transform, so nothing about the encoding changes. 'vertical' reads top-to-bottom; 'vertical-reversed' reads bottom-to-top, useful when the printed label itself will be mounted upside down relative to the other. barWidth always derives the bar-length axis (width, regardless of orientation) — height is now the fixed bar-thickness axis in every orientation.`
...
export const barcodeQrcodeSection: Node[] = [
  ...
  group({ direction: 'row', gap: 24, mainAlign: 'center', alignSelf: 'stretch' }, [
    group({ direction: 'column', gap: 6, crossAlign: 'center' }, [
      barcode({ value: 'VERTICAL-90', symbology: 'code128', orientation: 'vertical', barWidth: 3, height: 70 }),
      text({ content: "orientation: 'vertical'", fontFamily: UI_FONT, fontSize: 11, color: '#666666', align: 'center' }),
    ]),
    group({ direction: 'column', gap: 6, crossAlign: 'center' }, [
      barcode({ value: 'VERTICAL-N90', symbology: 'code128', orientation: 'vertical-reversed', barWidth: 3, height: 70 }),
