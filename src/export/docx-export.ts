...
import { pxToTwip } from './units.ts'
// Disabled alongside the watermark call in generateDocx() below — see there. Re-enable both
// together: uncomment this import, the two watermark-only docx imports below, and the whole
// "Watermark" section further down.
// import { HorizontalPositionRelativeFrom, TextWrappingType, VerticalPositionRelativeFrom } from 'docx'
// import type { ImageWatermark, TextWatermark, Watermark, WatermarkContent } from '../core/nodes.ts'
// import { resolveWatermarkInstances } from '../core/watermark-layout.ts'
// import { pxToEmu } from './units.ts'

export type DocxMetadata = { title?: string; author?: string; subject?: string; keywords?: string[] }
...
function warnUnsupportedNodeOnce(type: string): void {
  if (warnedNodeTypes.has(type)) return
...
function warnChartUnsupportedOnce(): void {
...
function warnTableBorderRadiusOnce(): void {
  if (warnedTableBorderRadius) return
...
function warnTableHeaderSeparatorOnce(): void {
  if (warnedTableHeaderSeparator) return
...
function warnTableRowBorderOnce(): void {
  if (warnedTableRowBorder) return
...
function separatorMainSize(node: SeparatorNode): number {
...
function isBold(fontWeight: number | string | undefined): boolean {
...
function textAlignment(align: 'left' | 'center' | 'right' | undefined): (typeof AlignmentType)[keyof typeof AlignmentType] {
  return align === 'center' ? AlignmentType.CENTER : align === 'right' ? AlignmentType.RIGHT : AlignmentType.LEFT
}

type RunChild = string | (typeof PageNumber)[keyof typeof PageNumber]
...
function runChildren(text: string, allowPageNumberSentinels: boolean): RunChild[] | undefined {
  if (!allowPageNumberSentinels || (!text.includes(PAGE_NUMBER_SENTINEL) && !text.includes(TOTAL_PAGES_SENTINEL))) return undefined
...
function textRun(text: string, style: RunStyle, allowPageNumberSentinels = false): TextRun {
...
function richTextRunStyle(paragraph: RichTextNode, run: RichTextRun): RunStyle {
  return {
...
function separatorParagraph(node: SeparatorNode): Paragraph {
...
// --- Image fetching: works for absolute URLs, relative paths, and data: URIs alike, same rationale
// as pdf-render.ts's <img>-based loading (see its header comment) — just via fetch() instead of the
// DOM Image element, since there's no canvas/DOM step needed here. ---

type DocxImageType = 'jpg' | 'png' | 'gif' | 'bmp'

function imageTypeFromMime(mime: string | null | undefined): DocxImageType | undefined {
  switch (mime) {
    case 'image/jpeg':
    case 'image/jpg':
      return 'jpg'
...
function imageTypeFromExtension(src: string): DocxImageType | undefined {
  const match = /\.(jpe?g|png|gif|bmp)(?:[?#]|$)/i.exec(src)
  if (match === null) return undefined
  const ext = match[1]!.toLowerCase()
  return ext === 'jpg' || ext === 'jpeg' ? 'jpg' : (ext as DocxImageType)
}

let warnedSvgImage = false
async function fetchImageData(src: string): Promise<{ data: Uint8Array; type: DocxImageType } | null> {
...
const NO_IMAGE_OUTLINE = { type: 'noFill' } as const
...
function resolveBoxSize(width: number | undefined, height: number | undefined, aspectRatio: number | undefined, availableWidthPx: number): { width: number; height: number } {
...
async function imageNodeParagraph(node: ImageNode, availableWidthPx: number): Promise<Paragraph> {
  ...
  if (image === null) return new Paragraph({})
  return new Paragraph({
...
async function rasterizeChart(node: ChartNode, widthPx: number, heightPx: number): Promise<{ data: Uint8Array; widthPx: number; heightPx: number } | null> {
...
async function chartNodeParagraph(node: ChartNode, availableWidthPx: number): Promise<Paragraph> {
  const { width, height } = resolveBoxSize(node.width, node.height, node.aspectRatio, availableWidthPx)
...
function drawBarcodeContentCanvas(ctx: OffscreenCanvasRenderingContext2D, node: BarcodeNode, pattern: BarPattern, contentWidth: number, contentHeight: number, scale: number): void {
  ...
  pattern.runs.forEach((runLength, i) => {
    ...
    if (i % 2 === 0) ctx.fillRect(cursor * scale, 0, runWidth * scale, barHeight * scale)
    cursor += runWidth
  })
  if (showText) {
...
function cellBorders(sides: BorderSides, lines: { inner: IBorderOptions; outer: IBorderOptions }): ITableCellBorders {
  const pick = (s: BorderSide): IBorderOptions => (s === 'outer' ? lines.outer : s === 'inner' ? lines.inner : NONE_BORDER)
...
function verticalAlignFor(align: CrossAlign | 'start' | 'center' | 'end' | undefined): (typeof VerticalAlignTable)[keyof typeof VerticalAlignTable] {
  return align === 'center' ? VerticalAlignTable.CENTER : align === 'end' ? VerticalAlignTable.BOTTOM : VerticalAlignTable.TOP
}

function shadingFor(color: string | undefined): { type: (typeof ShadingType)[keyof typeof ShadingType]; fill: string } | undefined {
  return color === undefined ? undefined : { type: ShadingType.CLEAR, fill: resolveExportColor(color) }
}

// --- Node -> block translation ---

async function nodeToBlocks(node: Node, widthPx: number): Promise<(Paragraph | DocxTable)[]> {
  switch (node.type) {
    case 'text':
...
function spacerParagraph(gapPx: number): Paragraph {
  return new Paragraph({ spacing: { before: 0, after: 0, line: pxToTwip(gapPx), lineRule: LineRuleType.EXACT } })
}

async function columnGroupToBlocks(node: GroupNode, widthPx: number): Promise<(Paragraph | DocxTable)[]> {
  ...
  results.forEach((childBlocks, i) => {
    if (i > 0 && gap > 0) blocks.push(spacerParagraph(gap))
    blocks.push(...childBlocks)
  })
  return blocks
}

function childCrossAlign(node: Node, fallback: CrossAlign): CrossAlign {
...
async function rowGroupToTable(node: GroupNode, widthPx: number): Promise<DocxTable> {
  const gap = node.gap ?? 0
...
async function containerToTable(node: ContainerNode, widthPx: number): Promise<DocxTable> {
...
async function tableNodeToTable(node: TableNode, widthPx: number): Promise<DocxTable> {
  const columnCount = node.columns.length
  const totalRows = node.rows.length
  const innerMode: TableBorderLineMode = node.border === undefined ? 'none' : (node.border.inner?.mode ?? 'all')
  const outerMode: TableBorderLineMode = node.border === undefined ? 'none' : (node.border.outer?.mode ?? 'all')
  if (node.border?.outer?.borderRadius !== undefined) warnTableBorderRadiusOnce()
  if (node.border?.headerSeparator) warnTableHeaderSeparatorOnce()
  if (node.rows.some(r => r.topBorder !== undefined || r.bottomBorder !== undefined)) warnTableRowBorderOnce()
  ...
  const rows = await Promise.all(
    node.rows.map(async (row, r) => {
      if (row.kind === 'header') {
        if (row.cells !== undefined) return docxRow(row.cells, undefined, r, totalRows, columnCount, colWidthsPx, innerMode, outerMode, borderLines, node.columns, tableCellPadding)
...
function cellMargins(paddingPx: number): { top: number; bottom: number; left: number; right: number } {
...
async function docxRow(
  cells: TableCell[],
  row: { background?: string; verticalAlign?: 'start' | 'center' | 'end' } | undefined,
  r: number,
  totalRows: number,
  columnCount: number,
  colWidthsPx: number[],
  ...
  borderLines: { inner: IBorderOptions; outer: IBorderOptions },
  columns: TableColumn[],
  tableCellPadding: number,
): Promise<DocxTableRow> {
  const children = await Promise.all(
    cells.map(async (cell, i) => {
      ...
      const fullRectSides: BorderSides = { top: 'outer', bottom: 'outer', left: 'outer', right: 'outer' }
      const borders =
...
function resolveHeaderFooterContent(content: HeaderFooterContent): Node {
  return typeof content === 'function' ? content(PLACEHOLDER_CTX) : content
}

async function headerFooterBlocks(content: HeaderFooterContent, widthPx: number): Promise<(Paragraph | DocxTable)[]> {
...
export async function generateDocx(doc: PageDef, metadata?: DocxMetadata): Promise<Uint8Array> {
