...
import type { QrcodeNode } from '../core/nodes.ts'
import { BASE_ELEMENT_STYLE } from '../render/reset.ts'
import { toPdfRect, resolvePdfColor } from '../render/pdf-render.ts'
import { buildQrMatrix, qrcodeRunsForRow } from '../render/qrcode-encode.ts'

type Rendered = Extract<RenderedNode, { type: 'qrcode' }>
...
function resolveHeight(node: QrcodeNode, width: number): number {
...
export function qrcodeNaturalWidth(node: QrcodeNode, availableWidth: number): number {
...
function layout(node: QrcodeNode, width: number): Rendered {
  ...
  for (let row = 0; row < matrix.moduleCount; row++) {
    for (const run of qrcodeRunsForRow(matrix, row)) {
      const rect = toPdfRect(
        x + (quietZone + run.startCol) * moduleWidth,
        y + (quietZone + row) * moduleHeight,
        run.length * moduleWidth,
        moduleHeight,
      )
