...
import type { ContainerBorder, LineStyle, TableBorderLineMode, TableNode } from '../../core/nodes.ts'
import type { RenderedTableRow } from '../../core/geometry.ts'
import { BORDER_EPSILON } from '../../render/interval-utils.ts'

export type LineStyleResolved = { thickness: number; color: string; style: LineStyle }
export type ResolvedBorderGroup = { mode: TableBorderLineMode } & LineStyleResolved

function resolveContainerBorder(b: ContainerBorder): LineStyleResolved {
...
export function resolveBorderLine(border: TableNode['border'], group: 'inner' | 'outer'): ResolvedBorderGroup {
  if (border === undefined) return { mode: 'none', thickness: 1, color: '#000000', style: 'solid' }
...
export function resolveOuterBorderRadius(border: TableNode['border']): number {
  return border?.outer?.borderRadius ?? 0
}

function resolveHeaderSeparator(headerSeparator: ContainerBorder | boolean | undefined, inner: LineStyleResolved): LineStyleResolved | undefined {
  if (headerSeparator === undefined || headerSeparator === false) return undefined
  if (headerSeparator === true) return inner
  return resolveContainerBorder(headerSeparator)
}

type RowOverride = { y: number; line: LineStyleResolved }
...
function collectRowOverrides(rows: RenderedTableRow[], originY: number): RowOverride[] {
  const overrides: RowOverride[] = []
  for (const row of rows) {
    if (row.topBorder !== undefined) overrides.push({ y: originY + row.box.y, line: resolveContainerBorder(row.topBorder) })
    if (row.bottomBorder !== undefined) overrides.push({ y: originY + row.box.y + row.box.height, line: resolveContainerBorder(row.bottomBorder) })
  }
  return overrides
}
...
export function createHorizontalLineStyler(args: {
  ...
  roundOuter: boolean
}): { styler: HorizontalLineStyler; candidateYs: number[] } {
  ...
  for (let i = 0; i < rows.length - 1; i++) candidateYs.add(originY + rows[i]!.box.y + rows[i]!.box.height)
  for (const o of overrides) candidateYs.add(o.y)
  if (headerBoundaryY !== undefined) candidateYs.add(headerBoundaryY)

  const near = (a: number, b: number) => Math.abs(a - b) < BORDER_EPSILON

  const styler: HorizontalLineStyler = y => {
    const override = overrides.find(o => near(o.y, y))
    if (override !== undefined) return override.line
    if (headerBoundaryY !== undefined && near(headerBoundaryY, y)) return headerSep!
    const isOuterY = near(y, tableTop) || near(y, tableBottom)
    if (isOuterY) return roundOuter ? null : outerH ? outer : null
