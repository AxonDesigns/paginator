...
function drawTableBorders(
  ...
  for (const lineY of candidateYs) {
    ...
    for (const [segStart, segEnd] of segments) drawGridSegment(doc, 'horizontal', drawY, segStart, segEnd, resolved.thickness, resolvePdfColor(resolved.color), resolved.style)
  }
  ...
  for (const { x: lineX, line } of vLines) {
    ...
    for (const [segStart, segEnd] of segments) drawGridSegment(doc, 'vertical', drawX, segStart, segEnd, line.thickness, resolvePdfColor(line.color), line.style)
  }
}
