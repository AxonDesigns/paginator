...
function rvwrapWidth(node: RichTextNode): number {
  const prepared = preparedFor(node)
...
export function richTextNaturalWidth(node: RichTextNode): number {
  if (isSideways(node.orientation)) return fullLines(node, rvwrapWidth(node)).length * node.lineHeight
  return rvwrapWidth(node)
}

function measureHeight(node: RichTextNode, width: number): number {
  if (isSideways(node.orientation)) return rvwrapWidth(node)
...
function appendRuns(container: HTMLElement, rendered: Rendered): void {
...
function drawRuns(rendered: Rendered, x: number, y: number, ctx: PdfRenderCtx, baselineFromTopPt: number, lineHeightPt: number): void {
  const node = rendered.node
