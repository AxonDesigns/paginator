...
import { drawChartAreaFill, drawChartCircle, drawChartLine, drawChartPath, drawChartPathStroke, drawChartText } from './pdf.ts'

function seriesKind(s: ChartSeries): 'bar' | 'line' | 'points' {
  return s.kind ?? 'bar'
}

function barSeriesWithIndex(series: ChartSeries[]): { s: ChartSeries; i: number }[] {
...
export function drawCategoricalChart(ctx: PdfRenderCtx, node: CategoricalChartNode, plot: ChartBox, originX: number, originY: number): void {
