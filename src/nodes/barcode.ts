...
import type { BarcodeNode } from '../core/nodes.ts'
...
function encodeNode(node: BarcodeNode): BarPattern {
...
function barGeometry(node: BarcodeNode, pattern: BarPattern, boxWidth: number, boxHeight: number) {
...
function drawBarcodeContentPdf(doc: PDFKit.PDFDocument, node: BarcodeNode, pattern: BarPattern, contentWidth: number, contentHeight: number): void {
  const { quietZone, showText, textSize, barHeight, moduleWidth } = barGeometry(node, pattern, contentWidth, contentHeight)
  ...
  pattern.runs.forEach((runLength, i) => {
    const runWidth = runLength * moduleWidth
    if (i % 2 === 0) doc.rect(cursor, 0, runWidth, barHeight).fill(barColor)
    cursor += runWidth
  })
