...
export function splitNode(node: Node, width: number, availableHeight: number): SplitOutcome<Node> {
  ...
  if (def.split === undefined) return null
  ...
  const rendered = { ...result.rendered, node: { ...result.rendered.node, __splitGroupId: splitGroupId } } as RenderedNode
  const rest = result.rest === null ? null : ({ ...result.rest, __splitGroupId: splitGroupId } as Node)
