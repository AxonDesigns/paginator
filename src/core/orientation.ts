...
export const VALID_ORIENTATIONS: readonly Orientation[] = ['horizontal', 'horizontal-reversed', 'vertical', 'vertical-reversed']
...
export function isSideways(orientation: Orientation | undefined): boolean {
...
export function isSplittableOrientation(orientation: Orientation | undefined): boolean {
  return !isSideways(orientation)
}

export function orientationAngle(orientation: Orientation | undefined): 0 | 90 | -90 | 180 {
  switch (orientation) {
    case 'vertical':
      return 90
...
export function validateOrientation(orientation: Orientation | undefined, caller: string): void {
...
export function orientedBoxSize(orientation: Orientation | undefined, naturalWidth: number, naturalHeight: number): { width: number; height: number } {
...
export function naturalContentSize(orientation: Orientation | undefined, finalWidth: number, finalHeight: number): { width: number; height: number } {
 ...
 */
export function cssOrientationTransform(
 ...
 */
export function svgOrientationTransform(orientation: Orientation | undefined, finalBoxWidth: number, finalBoxHeight: number): string | undefined {
  const angle = orientationAngle(orientation)
  if (angle === 0) return undefined
  if (angle === 90) return `translate(${finalBoxWidth} 0) rotate(90)`
  if (angle === -90) return `translate(0 ${finalBoxHeight}) rotate(-90)`
 ...
 */
export function applyPdfOrientationTransform(
  doc: PDFKit.PDFDocument,
  orientation: Orientation | undefined,
  xPt: number,
  yPt: number,
  finalWidthPt: number,
  finalHeightPt: number,
): void {
  switch (orientation) {
    case 'vertical':
 ...
 */
export function applyCanvasOrientationTransform(
  ctx: OffscreenCanvasRenderingContext2D,
  orientation: Orientation | undefined,
  scaledFinalWidth: number,
  scaledFinalHeight: number,
): void {
  switch (orientation) {
    case 'vertical':
