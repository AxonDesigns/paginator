...
export function resolveMarkerRadii(radius: number = MARKER_RADIUS): { radius: number; ringRadius: number } {
...
export function resolveBubbleRadius(size: number, sizeMin: number, sizeMax: number, scaleType: 'sqrt' | 'linear', range: readonly [number, number]): number {
  const [rMin, rMax] = range
  if (sizeMax <= sizeMin) return rMin
  const t = Math.min(1, Math.max(0, (size - sizeMin) / (sizeMax - sizeMin)))
  if (scaleType === 'linear') return rMin + t * (rMax - rMin)
...
export function ringSliceColor(node: { colors?: string[] }, ring: { colors?: string[] }, slice: { color?: string }, si: number): string {
...
export function offsetEdgePoint(cx: number, cy: number, angleDeg: number, halfGapPx: number, sign: 1 | -1, radius: number): [number, number] {
...
export function offsetApex(cx: number, cy: number, startAngleDeg: number, endAngleDeg: number, halfGapPx: number): [number, number] {
...
export type RingSliceAngle = { start: number; end: number }
...
export function resolveRingSliceAngles(slices: ChartRingSlice[], parentArcs: RingSliceAngle[] | null): RingSliceAngle[] {
  ...
  const totalByParent = new Map<number, number>()
  for (const s of slices) totalByParent.set(s.parentIndex!, (totalByParent.get(s.parentIndex!) ?? 0) + s.value)
  const cursorByParent = new Map<number, number>()
  return slices.map(s => {
...
export type RingRadii = { innerR: number; outerR: number }
...
export function resolveRingRadii(ringCount: number, innerRadiusRatio: number, outerRadius: number): RingRadii[] {
...
export function legendEntriesFor(node: ChartNode): LegendEntry[] {
  ...
  if (node.chartKind === 'gantt') return []
  ...
  if (node.chartKind === 'treemap') return []
...
export function resolveShowLegend(node: ChartNode, entryCount: number): boolean {
  ...
  if (node.chartKind === 'radial') return true
  if (node.chartKind === 'gantt') return false
  return entryCount > 1
}
...
export function resolveDomainFromExtent(rawMin: number, rawMax: number, view: ChartViewConfig): { dataMin: number; dataMax: number } {
...
export type GanttRow = { kind: 'header'; label: string } | { kind: 'task'; task: ChartGanttTask }
...
// headers at all.
export function resolveGanttRows(tasks: ChartGanttTask[], showGroupHeaders: boolean): GanttRow[] {
  if (!showGroupHeaders) return tasks.map(task => ({ kind: 'task', task }))
  const rows: GanttRow[] = []
...
export function resolveGanttGroupStyle(
  node: { groupHeaderColor?: string; groupHeaderBackground?: string; groups?: Record<string, ChartGanttGroupStyle> },
  label: string,
...
export function resolveGanttTaskLabelColor(node: { taskLabelColor?: string }, task: { labelColor?: string }): string {
...
export function radarSpokeAngle(i: number, spokeCount: number): number {
...
export function radarPolygonPoints(cx: number, cy: number, values: number[], domainMin: number, domainMax: number, maxRadius: number): [number, number][] {
...
export function candlestickGeometry(candle: ChartCandle, centerX: number, scale: (value: number) => number, candleWidth: number): CandlestickGeometry {
...
function worstAspectRatio(areas: number[], w: number): number {
...
function squarifyRows(areas: number[], box: ChartBox): ChartBox[] {
  const rects: ChartBox[] = []
  let remaining = box
  let items = areas
...
export function squarifyTreemap(items: readonly { value: number }[], box: ChartBox): ChartBox[] {
  const zeroRect: ChartBox = { x: box.x, y: box.y, width: 0, height: 0 }
  if (items.length === 0) return []
  const totalValue = items.reduce((acc, it) => acc + it.value, 0)
  if (totalValue <= 0) return items.map(() => zeroRect)
  ...
  const result: ChartBox[] = items.map(() => zeroRect)
