...
import type { PdfContext } from './pdf-render.ts'
import type { RichTextNode, RichTextRun, TextNode } from '../core/nodes.ts'
import { lookupFont, normalizeFontWeight, resolveStandardFontName } from './font-registry.ts'
import type { FontStyle, RegisteredFont } from './font-registry.ts'

export function resolveFontOrThrow(ctx: PdfContext, family: string, weight: number, style: FontStyle): string {
  ...
  throw new Error(
    `[paginator] generatePdf(): no font registered for family "${family}", weight ${weight}, style "${style}", and it is not one of pdfkit's ` +
...
export function ensureRegisteredFont(ctx: PdfContext, font: RegisteredFont): string {
...
export function resolveRunFont(ctx: PdfContext, run: RichTextRun, node: RichTextNode): string {
...
export function textNodeFontString(node: TextNode): string {
...
export function richTextNodeFontString(node: RichTextNode): string {
  const style = node.fontStyle === 'italic' ? 'italic ' : ''
  const weight = node.fontWeight ?? 400
  return `${style}${weight} ${node.fontSize}px ${node.fontFamily}`
}
...
export function getMetricsCanvasCtx(): OffscreenCanvasRenderingContext2D {
...
// font CSS string.
export function measureFontMetricsPx(fontCss: string): { ascentPx: number; descentPx: number } {
