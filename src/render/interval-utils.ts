...
// uninterrupted line.
export function subtractIntervals(full: readonly [number, number], holes: readonly (readonly [number, number])[]): [number, number][] {
  const sorted = [...holes].map(([a, b]): [number, number] => (a <= b ? [a, b] : [b, a])).sort((a, b) => a[0] - b[0])
  const result: [number, number][] = []
  let cursor = full[0]
  for (const [start, end] of sorted) {
    if (end <= cursor) continue
    if (start > cursor) result.push([cursor, Math.min(start, full[1])])
    cursor = Math.max(cursor, end)
    if (cursor >= full[1]) break
  }
  if (cursor < full[1]) result.push([cursor, full[1]])
