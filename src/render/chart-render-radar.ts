...
export function renderRadarChart(svg: SVGSVGElement, node: RadarChartNode, plot: ChartBox): void {
  ...
  if (gridlinesShow) {
    ...
    for (let i = 0; i < spokeCount; i++) {
      ...
      svg.appendChild(
        svgEl('line', { x1: cx, y1: cy, x2: cx + outerRadius * Math.cos(angle), y2: cy + outerRadius * Math.sin(angle), stroke: axisColor, 'stroke-width': 1 }),
      )
    }
  }
  ...
  if (axisShow) {
    ...
    categories.forEach((category, i) => {
      ...
      svg.appendChild(
        svgText(category, cx + labelR * cosA, cy + labelR * Math.sin(angle) + categoryBaselineOffset, { fontSize: categoryFontSize, fontFamily, fill: tickColor, 'text-anchor': anchor }),
      )
    })
  }
  ...
  series.forEach((s, si) => {
    ...
    svg.appendChild(
      svgEl('path', { d: polygonPath(points), fill: 'none', stroke: colors[si]!, 'stroke-width': lineStrokeWidth, 'stroke-linejoin': 'round' }),
    )
    if (markerRadius > 0) {
