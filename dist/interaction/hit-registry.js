...
export function buildHitRegistry(result) {
...
function descendPath(entry, x, y, path) {
    path.push(entry);
    let best = null;
    for (const child of entry.children) {
        if (!boxContains(child.rendered.box, x, y))
            continue;
        if (best === null || child.rendered.box.width * child.rendered.box.height < best.rendered.box.width * best.rendered.box.height)
            best = child;
    }
...
function findPath(registry, pageNumber, x, y) {
...
function findAlongPath(path, predicate) {
    for (let i = path.length - 1; i >= 0; i--) {
        if (predicate(path[i].rendered.node))
            return path[i];
    }
    return null;
}
 ...
 */
export function hitTest(registry, pageNumber, x, y) {
...
export function toTypeList(value) {
    if (value === undefined)
        return [];
...
function typeMatches(dragTypes, accepts) {
 ...
 */
export function hitTestDroppable(registry, pageNumber, x, y, dragTypes = []) {
    const path = findPath(registry, pageNumber, x, y);
    if (path === null)
        return null;
