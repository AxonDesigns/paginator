...
import { AXIS_COLOR, CHART_FONT_FAMILY, GRIDLINE_COLOR, INK_MUTED, barPath, estimateChartTextWidth, niceTickValues, resolveColor, resolveDomainFromExtent, resolveGanttGroupStyle, resolveGanttRows, resolveGanttTaskLabelColor, roundedRectPath, textBaselineOffset, } from "../../render/chart-geometry.js";
import { drawChartLine, drawChartPath, drawChartText } from "./pdf.js";
const ROW_FONT_SIZE = 11;
export function drawGanttChart(ctx, node, plot, originX, originY) {
    ...
    rows.forEach((row, ri) => {
        ...
        taskIndex++;
        drawChartText(ctx, task.label, plotLeft - 8, rowCenterY + rowBaselineOffset, { fontSize: ROW_FONT_SIZE, color: resolvePdfColor(resolveGanttTaskLabelColor(node, task)), anchor: 'end', bold: false, fontFamily }, originX, originY);
