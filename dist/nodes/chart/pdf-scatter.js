...
import { AXIS_COLOR, CHART_FONT_FAMILY, GRIDLINE_COLOR, INK_MUTED, MARKER_RADIUS, SURFACE_COLOR, estimateChartTextWidth, niceTickValues, resolveBubbleRadius, resolveColor, resolveDomainFromExtent, resolveMarkerRadii, textBaselineOffset, } from "../../render/chart-geometry.js";
import { drawChartCircle, drawChartLine, drawChartText } from "./pdf.js";
export function drawScatterChart(ctx, node, plot, originX, originY) {
    ...
    const formatYTick = yAxis.formatTick ?? ((v) => Math.round(v).toLocaleString());
    ...
    const yScale = (v) => plotBottom - ((v - yMin) / (yMax - yMin)) * plotHeight;
    if (yGridlinesShow) {
    ...
    const allSizes = sizeScale !== undefined ? series.flatMap(s => s.points.map(p => p.size).filter((v) => v !== undefined)) : [];
