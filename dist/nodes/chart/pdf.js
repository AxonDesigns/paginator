...
import { PX_TO_PT, pxToPt, resolvePdfColor } from "../../render/pdf-render.js";
import { resolveChartFontName } from "../../render/pdf-fonts.js";
import { normalizeFontWeight } from "../../render/font-registry.js";
import { CHART_FONT_FAMILY, INK_SECONDARY, estimateChartTextWidth, legendEntriesFor, normalizeChartText, resolveShowLegend, resolveTitle, textBaselineOffset, truncateToWidth, wrapChartTextToWidth, } from "../../render/chart-geometry.js";
import { drawCategoricalChart } from "./pdf-categorical.js";
import { drawRadialChart } from "./pdf-radial.js";
import { drawScatterChart } from "./pdf-scatter.js";
import { drawGanttChart } from "./pdf-gantt.js";
import { drawRadarChart } from "./pdf-radar.js";
import { drawCandlestickChart } from "./pdf-candlestick.js";
import { drawTreemapChart } from "./pdf-treemap.js";
function chartToPagePoint(originX, originY, localX, localY) {
    return { x: pxToPt(originX + localX), y: pxToPt(originY + localY) };
}
export function drawChartLine(ctx, x1, y1, x2, y2, color, thickness, originX, originY) {
...
export function drawChartPathStroke(ctx, d, color, thickness, originX, originY) {
...
export function drawChartAreaFill(ctx, d, gx1, gy1, gx2, gy2, color, opacity, originX, originY) {
    const origin = chartToPagePoint(originX, originY, 0, 0);
    const doc = ctx.pdf.doc;
    doc.save();
    doc.translate(origin.x, origin.y);
...
export function drawChartCircle(ctx, cx, cy, r, color, originX, originY) {
...
export function drawChartPath(ctx, d, color, originX, originY, opacity = 1) {
    const origin = chartToPagePoint(originX, originY, 0, 0);
    const doc = ctx.pdf.doc;
    doc.save();
    doc.translate(origin.x, origin.y);
...
export function drawChartText(ctx, text, localX, localY, opts, originX, originY) {
    const doc = ctx.pdf.doc;
...
export function drawChartLegend(ctx, entries, box, orientation, fontSize, fontFamily, color, originX, originY) {
    const swatch = 10;
...
export function drawChartNode(rendered, x, y, ctx) {
    ...
    if (title !== null) {
        ...
        wrappedLines.forEach((line, li) => {
            drawChartText(ctx, line, width / 2, top + title.fontSize + 4 + li * lineHeight, { fontSize: title.fontSize, fontFamily, color: resolvePdfColor(title.color), anchor: 'middle', bold: false }, originX, originY);
        });
        top += band;
    }
