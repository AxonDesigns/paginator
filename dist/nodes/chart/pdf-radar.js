...
import { AXIS_COLOR, CHART_FONT_FAMILY, GRIDLINE_COLOR, INK_MUTED, LINE_STROKE_WIDTH, MARKER_RADIUS, SURFACE_COLOR, niceTickValues, polygonPath, radarPolygonPoints, radarSpokeAngle, resolveChartDomain, resolveColor, resolveLineFill, resolveMarkerRadii, textBaselineOffset, } from "../../render/chart-geometry.js";
import { drawChartCircle, drawChartLine, drawChartPath, drawChartPathStroke, drawChartText } from "./pdf.js";
export function drawRadarChart(ctx, node, plot, originX, originY) {
    ...
    if (axisShow) {
        ...
        categories.forEach((category, i) => {
            ...
            drawChartText(ctx, category, cx + labelR * cosA, cy + labelR * Math.sin(angle) + categoryBaselineOffset, { fontSize: categoryFontSize, color: tickColor, anchor, bold: false, fontFamily }, originX, originY);
        });
    }
...
function drawChartCirclePath(ctx, cx, cy, r, color, originX, originY) {
    if (r <= 0)
        return;
