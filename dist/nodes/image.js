...
import { embedImage, pxToPt, toPdfRect } from "../render/pdf-render.js";
...
// as a separator.
export function imageNaturalWidth(node, availableWidth) {
...
async function drawPdf(rendered, x, y, ctx) {
    ...
    if (needsClip || needsOpacity)
        doc.save();
    if (needsClip)
        doc.roundedRect(rect.x, rect.y, rect.width, rect.height, pxToPt(node.borderRadius)).clip();
    if (needsOpacity)
