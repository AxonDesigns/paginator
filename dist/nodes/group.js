...
import { drawPdfNode, isSplittable, layoutNodeFull, measureNodeHeight, naturalWidth, registerNode, renderNodeDom, splitNode, } from "../core/behavior.js";
import { resolveFlexWidths } from "../core/flex-widths.js";
import { separatorMainSize } from "./separator.js";
...
function rowChildKind(node) {
    if (node.type === 'separator' || node.type === 'page-break')
        return 'fixed'; // page-break inert as a row column — see nodes.ts
...
function resolveRowChildSizing(node, availableWidth) {
    if (node.type === 'separator')
    ...
    const flex = node.flex;
    if (flex === 'shrink')
        return { kind: 'fixed', size: shrinkWrapWidth(node, availableWidth) };
    if (flex === undefined && 'width' in node && node.width !== undefined)
...
function emptyContinuationFor(node, width) {
    const sizing = resolveRowChildSizing(node, width);
    const flex = sizing.kind === 'fixed' ? `${sizing.size}px` : sizing.weight;
...
function rowHasFlexChild(node) {
    return node.children.some(c => rowChildKind(c) === 'flex');
}
function sumFixedRowWidth(node, width) {
...
function shrinkWrapWidth(node, width) {
    if (node.type === 'group') {
...
export function childCrossWidthInColumn(node, width) {
...
function distributeFreeSpace(freeSpace, gap, n, mainAlign) {
    ...
    if (n > 1) {
        if (mainAlign === 'center')
            ...
            leading = freeSpace;
        else if (mainAlign === 'space-between')
            between = gap + freeSpace / (n - 1);
...
export function layoutColumn(node, width, targetHeight) {
    const gap = node.gap ?? 0;
...
export function layoutRow(node, width) {
    const gap = node.gap ?? 0;
    const crossAlign = node.crossAlign ?? 'start';
    const mainAlign = node.mainAlign ?? 'start';
    const n = node.children.length;
...
export function subtreeHasPageBreak(node) {
    if (node.type === 'page-break')
        return true;
    if (node.type !== 'group' || node.direction !== 'column')
        return false;
    return node.children.some(subtreeHasPageBreak);
}
// --- Column-group splitting across a page boundary ---
function columnGroupSplit(node, width, availableHeight) {
    ...
    if (fitted.length === 0)
        return null;
    const rest = restChildren.length === 0 ? null : { ...node, children: restChildren };
    return {
...
function rowGroupSplit(node, width, availableHeight) {
    ...
    if (rowConsumedHeight <= 0)
        return null; // row-level orphan: not even one column produced anything
    const rest = anyoneContinues ? { ...node, children: restChildren } : null;
    return {
...
function renderDom(rendered, x, y, ctx) {
    ...
    for (const child of rendered.children)
        renderNodeDom(child, ctx.originX, ctx.originY, ctx);
}
// No PDF equivalent beyond recursing — a group's wrapper box is DOM-devtools-only paint, nothing to
// draw in the PDF.
async function drawPdf(rendered, _x, _y, ctx) {
    for (const child of rendered.children)
        await drawPdfNode(child, ctx.originX, ctx.originY, ctx.pdf);
}
registerNode('group', {
    ...
    split: (node, width, availableHeight) => {
        if (node.direction === 'column')
            return columnGroupSplit(node, width, availableHeight);
        if (node.direction === 'row' && node.splitColumns === true)
            return rowGroupSplit(node, width, availableHeight);
        return null;
    },
