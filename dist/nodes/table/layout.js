...
import { layoutNodeFull, measureNodeHeight } from "../../core/behavior.js";
import { resolveFlexWidths } from "../../core/flex-widths.js";
import { childCrossWidthInColumn } from "../group.js";
const EPSILON = 0.01;
...
function layoutCell(cell, column, colWidth, tablePadding) {
...
function prepareRowCells(row, columns, colWidths, cellPadding) {
...
export function resolveRowHeights(rows, columns, colWidths, cellPadding) {
    const fullWidth = colWidths.reduce((a, b) => a + b, 0);
    const preparedPerRow = [];
    const heights = [];
    for (const row of rows) {
        if (row.kind === 'header') {
...
export function layoutRows(rows, columns, colWidths, cellPadding) {
    const fullWidth = colWidths.reduce((a, b) => a + b, 0);
    ...
    const renderCells = (prepared, r) => prepared.map(cell => {
        ...
        const dy = cell.verticalAlign === 'center' ? (availableHeight - cell.naturalHeight) / 2 : cell.verticalAlign === 'end' ? availableHeight - cell.naturalHeight : 0;
...
function hasRemainingContentAtDepth(rows, depth) {
    for (const row of rows) {
        if (row.kind === 'header') {
            if (row.depth <= depth)
                return false;
...
export function splitTable(node, width, availableHeight) {
    ...
    while (i < dataRows.length) {
        ...
        let clusterEnd = i;
        while (clusterEnd + 1 < dataRows.length && isAtomicWithNext(dataRows[clusterEnd]))
            clusterEnd++;
        const clusterHeight = sumRange(dataHeights, i, clusterEnd + 1);
        if (dataHeight + clusterHeight > remainderForData + EPSILON)
            break;
    ...
    if (fittedData.length === 0)
        return null; // orphan: not even one cluster fit (header rows alone don't count as progress)
