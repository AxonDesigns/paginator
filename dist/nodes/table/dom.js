...
function renderTableBorders(node, rendered, colWidths, colX, originX, originY, x, y, container, roundOuter) {
    ...
    for (const lineY of candidateYs) {
        ...
        for (const [segStart, segEnd] of segments)
            container.appendChild(gridSegmentDiv('horizontal', drawY, segStart, segEnd, resolved.thickness, resolved.color, resolved.style));
    }
    const vLines = [];
    ...
    for (const { x: lineX, line } of vLines) {
        ...
        for (const [segStart, segEnd] of segments)
            container.appendChild(gridSegmentDiv('vertical', drawX, segStart, segEnd, line.thickness, line.color, line.style));
    }
}
