...
import { toPdfRect } from "../render/pdf-render.js";
...
export function svgNaturalWidth(node, availableWidth) {
...
function renderDom(rendered, x, y, ctx) {
    ...
    const el = document.importNode(root, true);
...
function drawPdf(rendered, x, y, ctx) {
    ...
    SVGtoPDF(doc, node.markup, rect.x, rect.y, {
        ...
        warningCallback: (w) => console.warn(`[paginator] svg node: ${w}`),
    });
    if (needsOpacity)
        doc.restore();
}
