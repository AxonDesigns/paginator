...
import { AXIS_COLOR, CHART_FONT_FAMILY, GRIDLINE_COLOR, INK_MUTED, estimateChartTextWidth, niceTickValues, resolveColor, resolveDomainFromExtent, resolveGanttGroupStyle, resolveGanttRows, resolveGanttTaskLabelColor, roundedRectPath, textBaselineOffset, } from "./chart-geometry.js";
import { svgEl, svgText } from "./chart-render.js";
const ROW_FONT_SIZE = 11;
export function renderGanttChart(svg, node, plot) {
    ...
    const formatXTick = xAxis.formatTick ?? ((v) => Math.round(v).toLocaleString());
    ...
    const xScale = (v) => plotLeft + ((v - xMin) / (xMax - xMin)) * plotWidth;
    ...
    rows.forEach((row, ri) => {
        ...
        if (row.kind === 'header') {
            ...
            svg.appendChild(svgText(row.label, plot.x + 8, rowCenterY + rowBaselineOffset, { fontSize: ROW_FONT_SIZE, fontFamily, fill: headerColor, 'font-weight': 700, 'text-anchor': 'start' }));
            return;
        }
        const task = row.task;
        const color = resolveColor(task.color, node.colors, taskIndex);
        taskIndex++;
        svg.appendChild(svgText(task.label, plotLeft - 8, rowCenterY + rowBaselineOffset, { fontSize: ROW_FONT_SIZE, fontFamily, fill: resolveGanttTaskLabelColor(node, task), 'text-anchor': 'end' }));
