...
export function resolvePdfColor(css) {
...
function normalizeCssColor(css) {
    ...
    if (result === COLOR_PARSE_SENTINEL && css.toLowerCase() !== COLOR_PARSE_SENTINEL)
        return null;
    if (result.startsWith('#'))
        return result;
    const rgbaMatch = /^rgba?\(\s*(\d+)\s*,\s*(\d+)\s*,\s*(\d+)\s*(?:,\s*[\d.]+\s*)?\)$/.exec(result);
    if (rgbaMatch === null)
        return null; // unreachable in practice — fillStyle only ever normalizes to #rrggbb or rgba(...)
...
export function resolveObjectFitRects(mode, iw, ih, bw, bh) {
    if (mode === 'fill')
        return { sx: 0, sy: 0, sw: iw, sh: ih, dx: 0, dy: 0, dw: bw, dh: bh };
    if (mode === 'none')
        return { sx: 0, sy: 0, sw: iw, sh: ih, dx: (bw - iw) / 2, dy: (bh - ih) / 2, dw: iw, dh: ih };
    if (mode === 'cover') {
...
async function rasterizeImageToPng(node, boxWidthPx, boxHeightPx) {
    ...
    if (ctx2d === null)
        throw new Error('[paginator] generatePdf(): could not acquire a 2D context for image rasterization.');
...
export async function embedImage(ctx, node, boxWidthPx, boxHeightPx) {
...
const watermarkTextRasterCache = new Map();
async function rasterizeWatermarkText(text, fontCss, color, widthPx, ascentPx, descentPx) {
    ...
    const ctx2d = canvas.getContext('2d');
    if (ctx2d === null)
        throw new Error('[paginator] generatePdf(): could not acquire a 2D context for watermark text rasterization.');
...
async function drawWatermark(ctx, watermark, pageWidthPx, pageHeightPx) {
 ...
 */
export async function generatePdf(result, fonts, metadata) {
    ...
    for (const pdfPage of result.pages) {
        ...
        if (pdfPage.header !== null)
            await drawPdfNode(pdfPage.header, headerOriginX, headerOriginY, ctx);
        for (const node of pdfPage.body)
        ...
        if (pdfPage.watermark !== null)
            await drawWatermark(ctx, pdfPage.watermark, pageSize.width, pageSize.height);
