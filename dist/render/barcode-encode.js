...
function code128Symbols(value, useSetC) {
    if (useSetC) {
        const symbols = [];
...
export function encodeCode128(value) {
    if (value.length === 0)
        throw new Error('[paginator] encodeCode128(): value must not be empty.');
...
const EAN13_R = EAN13_L.map(bits => [...bits].map(b => (b === '1' ? '0' : '1')).join(''));
const EAN13_G = EAN13_R.map(bits => [...bits].reverse().join(''));
...
export function encodeEan13(value, checkDigitMode = 'auto') {
    if (!/^\d+$/.test(value))
        throw new Error(`[paginator] encodeEan13(): value must contain only digits, got ${JSON.stringify(value)}.`);
...
export function encodeCode39(value, checkDigitMode = 'omit') {
    const normalized = value.toUpperCase();
    if (normalized.length === 0)
        throw new Error('[paginator] encodeCode39(): value must not be empty.');
...
export function encodeBarcodeValue(symbology, value, checkDigitMode) {
    switch (symbology) {
        case 'code128':
