...
import { resolveColumnWidths } from "../nodes/table/index.js";
import { findTables } from "./find-tables.js";
import { flattenNodeToText } from "./node-to-text.js";
import { toArgb } from "./export-color.js";
import { pxToExcelWidth, pxToPt } from "./units.js";
import { borderSides, borderStyleForThickness as sharedBorderStyleForThickness } from "./table-grid.js";
let warnedNestedCellContent = false;
function warnNestedCellContentOnce() {
    if (warnedNestedCellContent)
        return;
...
function textNodeFont(node) {
    const font = { size: pxToPt(node.fontSize), name: node.fontFamily.split(',')[0].trim().replace(/^["']|["']$/g, '') };
    if (node.fontWeight !== undefined && (node.fontWeight === 'bold' || Number(node.fontWeight) >= 600))
        font.bold = true;
    if (node.fontStyle === 'italic')
        font.italic = true;
    if (node.color !== undefined)
        font.color = { argb: toArgb(node.color) };
    if (node.textDecoration === 'underline')
        font.underline = true;
    if (node.textDecoration === 'line-through')
        font.strike = true;
    return font;
}
...
function runFont(paragraph, run) {
    const fontFamily = run.fontFamily ?? paragraph.fontFamily;
    const font = { size: pxToPt(run.fontSize ?? paragraph.fontSize), name: fontFamily.split(',')[0].trim().replace(/^["']|["']$/g, '') };
    ...
    if (fontWeight !== undefined && (fontWeight === 'bold' || Number(fontWeight) >= 600))
        font.bold = true;
    if (fontStyle === 'italic')
        font.italic = true;
    if (color !== undefined)
        font.color = { argb: toArgb(color) };
...
function applyCellContent(cell, node) {
    if (node.type === 'text') {
...
function borderStyleForThickness(thickness) {
...
function writeCell(sheet, excelRow, cell, arrayIndex, rowBackground, rowVerticalAlign, totalRows, columnCount, tableMode, tableBorderColor, tableBorderStyle) {
    ...
    if (colSpan > 1 || rowSpan > 1)
        sheet.mergeCells(excelRow, colStart + 1, rowEnd + 1, colEnd + 1);
    const target = sheet.getCell(excelRow, colStart + 1);
    if (cell.content !== undefined)
        applyCellContent(target, cell.content);
    ...
    table.rows.forEach((row, r) => {
        ...
        if (row.kind === 'header') {
            ...
            if (row.content !== undefined)
                applyCellContent(target, row.content);
...
export async function generateXlsx(doc, metadata) {
